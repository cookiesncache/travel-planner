...
## Spending file: mixed-currency + adversarial-review fixes — ✅ implemented (2026-06-23, v1.2.0)

Follow-up to the v1.1.0 spending-file work, after an adversarial best-practices review (the review workflow's verify stage was lost to a monthly spend limit, so the 22 raised findings were adjudicated inline). Confirmed fixes shipped:

- **Mixed-currency support (new).** The spending sheet gained `Currency` + `FX→home` + `Amount (home)` (formula) columns. Each booking keeps its **native** amount + currency (preserved, always visible); Total / Remaining / Still-due sum the **home-currency** column via `=IF(OR(native="",rate=""),"",native*rate)`. Home currency comes from intake (default USD); Claude fills FX rates via WebSearch at capture, user can override. Replaces the earlier "single currency, out of scope" limitation — international trips need this.
- **Runtime execution guidance.** `spending-integration.md` now tells Claude *how* to run the generator: resolve it via `${CLAUDE_PLUGIN_ROOT}`, ensure `openpyxl`, invoke with the Bash tool.
- **Recalc reframed.** "Mandatory recalc-validation" → "recommended pre-flight": every spreadsheet app recalcs on open, so users always see correct totals; the step is a dev-time formula-error check. The `soffice` command now converts into a **temp dir** so it can't overwrite the canonical file.
- **Generator hardening.** Dynamic formula ranges / summary position (no 200-row ceiling); amounts + rates coerced to numbers (unparseable → blank, not silent 0); missing `id` errors loudly; ID-keyed preservation now covers native amount, FX rate, and budget.
- **Drop-row mechanism spelled out.** Cancel/orphan/refund = remove from `## Bookings` and regenerate (preserving other inputs by ID), not a hand-deleted xlsx row.

Dismissed as false positives (verified inline): a "points + due-on-arrival" double-count (single-enum prevents it), falsy-id / re-add resurrection (IDs never reused), and README "recalc on edit" (true of live formulas). The hand-entered-booking-vs-Stop-hook gap is a pre-existing by-design limitation (the hook only guards connector writes), not a regression.
