...
### P1 — Logic gaps that break advertised behavior  *(7 of 7 fixed; P1-5 pending commit)*
...
### P2 — Honesty & polish  *(4 of 5 fixed; P2-5 remains, and is largely defused by the v0.7.2 ask-gate)*
...
**P2-5 · Export gate behavior inside subagents** *(gap; low reachability — largely defused by v0.7.2)*
Original concern: PreToolUse fires inside subagents where the parent's approval isn't visible, so the old **deny-gate** false-blocked a delegated approved export. The **v0.7.2 ask-gate removes that** — the gate no longer detects confirmation; it returns `ask` for any active-trip connector write, so "invisible parent approval" no longer causes a false-deny. Residual (untested): whether an `ask` raised from inside a subagent surfaces a usable permission prompt or misbehaves in a non-interactive subagent context. Reachability is low — the only shipped agent (`feasibility-check`) is read-only and never writes to a connector; a future Booking Intel agent would also read-and-report. *Fix when it becomes reachable:* verify `ask`-in-subagent behavior with `claude --debug`, and if it doesn't surface cleanly, either route connector writes through the main thread or add a SubagentStop mirror (already a Booking Intel dependency).
