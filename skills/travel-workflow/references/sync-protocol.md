...
- **Cancellation & orphan (subtractive reconcile):** reconciliation runs in the **main thread**, not an agent. (a) When the `booking-intel` agent reports a `cancelled` item — or the user cancels one — set its row to Status `cancelled` and update Last action. (b) During Step 1 reconcile, the main thread reads each connector and checks whether each `synced` row's remote ID still exists; if it's gone, set the row to `orphaned`. In both cases **surface the change and get the user's confirmation before editing the plan body** (gate 1): remove or flag the item, recalculate the Spending Tracker, and make any connector cleanup (deleting a leftover calendar event, closing a task) through the export gate (`ask`). Never silently drop a row — re-status it (rows are never deleted).
- **Dedup on import:** an item whose remote ID already appears in Sync State is already handled — if the matching row is `declined`, `untracked`, `cancelled`, or `orphaned`, suppress it (do not re-surface as new); otherwise reconcile its status. Do not re-surface it.
