...
- **Dedup on import — match the right identifier.** A **connector** item (calendar event, task) carries a remote ID, so an item whose remote ID already appears in Sync State is already handled. An **email** confirmation has a *confirmation number*, not a remote ID — Sync State's Remote ID column holds connector IDs, a different identifier space — so match its confirmation number against the plan's **Bookings / Spending Tracker** (`Confirmation #`), not the Remote ID column. Either way: if the match is a `declined`, `untracked`, `cancelled`, or `orphaned` row/item, suppress it (do not re-surface as new); otherwise reconcile its status. Suppression is keyed to the **same booking identity** — the same confirmation number / remote ID, or, for items with no stored number (e.g. `untracked`), the same vendor + dates. A **new** confirmation number for the same vendor + dates of a `cancelled`/`orphaned`/`declined` item is a *re-booking*, not a duplicate — surface it (the old row stays as history).

## Spending Tracker interaction

The Spending Tracker's first column is the item ID, joined to Sync State (see `itinerary-integration.md` for the format). Capturing a confirmed booking writes plan body, tracker row, and Sync State row in one authorized write. Declined, untracked, and needs-attention items never get tracker rows. **Removing a tracker row on cancel/orphan is item-level, not row-level:** drop the row and recalculate the total only when the *item* is truly gone — every connector row for that ID is `cancelled`/`orphaned` and it's removed from the plan body. An orphan on just one of several connector rows (e.g. the Notion copy deleted while the calendar event still stands) is a connector-cleanup matter, not a spend change — keep the tracker row. **Non-refundable exception:** when a truly-gone booking leaves a standing non-refundable charge, keep its tracker row, relabel the Item (e.g. "Hotel Shinjuku (cancelled, non-refundable)"), and set Amount to the forfeited amount so it still counts toward Total/Remaining; only a fully-refunded cancellation drops the row.
