...
`booking-intel` returns three lists, cross-referenced against the plan (email confirmations deduped by confirmation number against the plan's `## Bookings` list — not Sync State Remote IDs; `declined`/`untracked`/`cancelled`/`orphaned` items skipped):
