...
This governs **Step 5 itinerary export** — what trip *content* becomes calendar events. Before creating any events, present them in one native `AskUserQuestion` (gate 2) naming what will be created and in which calendar, and create only the ones the user selects — see `sync-protocol.md`. Create calendar events only for the following:
...
**Step 7 exception:** if the calendar is the chosen reminder capability, a user-confirmed reminder alert for a time-sensitive task (an `rm`-type item, e.g. "apply for visa") *may* be created as a calendar event. That is a reminder, not itinerary content — it is governed by `reminder-integration.md`, and the ban above applies only to Step 5 itinerary export. A timed-entry attraction may still have an associated prep task (e.g. "bring confirmation"); that task goes to the task app, not the calendar.
