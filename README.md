...
💰 Your trip's money lives in a **standalone spending spreadsheet** with live formulas — budget, total spent, what's left, and what's still due in person, all recalculating the moment you change an amount. It handles **multiple currencies**, converting each booking into your home currency while keeping the original foreign amounts. It opens and recalculates in LibreOffice, Apple Numbers, Google Sheets, or Excel (no proprietary software needed): you edit the amounts and the budget, the totals take care of themselves. The plan itself keeps just a one-line summary and a pointer to the file, so the numbers live in exactly one place and never drift between copies.
