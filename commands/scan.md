...
Present the agent's `missing` / `flagged` / `cancelled` digest and **ask before changing anything** — this command only surfaces findings. If no email app is connected, say so and stop.
